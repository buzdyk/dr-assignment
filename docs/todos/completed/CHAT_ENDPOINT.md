...
Vendors type a plain-English question and need a grounded answer backed by their own data. This todo is the entry point: the single server route that accepts the prompt, runs the AI↔tools loop, and returns what the UI renders. SSE streaming is deferred to [[SSE_AI_ENDPOINT]]; this first cut returns JSON once the orchestration settles so we can iterate on the meat without SSE framing in the way.
...
No `conversation_id`, no history array — stateless per-request. Multi-turn and persistence are deferred to [[../icebox/CHAT_POLISH]].
...
- SSE streaming → [[SSE_AI_ENDPOINT]]
- Multi-turn history, persistence → [[../icebox/CHAT_POLISH]]
- BYOK key supplied per-request → [[BYOK_KEY_HANDLING]] (for now the key comes from `ANTHROPIC_API_KEY` server env)
- Test coverage and fixture-driven RobotProvider → [[BACKEND_TESTS]]
