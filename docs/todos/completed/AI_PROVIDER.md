...
- Reads the API key from `ANTHROPIC_API_KEY` (server env, per [[../adr/002-BYOK_CLAUDE]]). Per-request BYOK is deferred to [[../backlog/BYOK_KEY_HANDLING]].
- Non-streaming calls for now (`messages.create` without `stream: true`). Streaming variants are the concern of [[../backlog/SSE_AI_ENDPOINT]].
...
- No fixture files, no scripting DSL. That lives in [[../backlog/BACKEND_TESTS]].
...
- Streaming/SSE variants → [[../backlog/SSE_AI_ENDPOINT]]
- Fixture-driven scripted responses, error injection, tool-call assertions → [[../backlog/BACKEND_TESTS]]
- Per-request BYOK key handling → [[../backlog/BYOK_KEY_HANDLING]]
- Prompt caching → future concern, ADR when it matters

## Related

- [[CHAT_ENDPOINT]] — the consumer of this interface
- [[QUERY_TOOLS]] — what `onToolCall` ultimately reaches
- [[../adr/002-BYOK_CLAUDE]]
- [[../adr/008-TEXT_TO_SQL]]
- [[../reading/01_CLAUDE_CAPABILITIES]]
- [[../backlog/BACKEND_TESTS]]
